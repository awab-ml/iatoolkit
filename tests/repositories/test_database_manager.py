...
class TestDatabaseManager:
    def setup_method(self):
        ...
        patcher_engine = patch('repositories.database_manager.create_engine', return_value=self.mock_engine)
        patcher_sessionmaker = patch('repositories.database_manager.sessionmaker', return_value=self.mock_sessionmaker)
        patcher_scoped_session = patch('repositories.database_manager.scoped_session',
                                       return_value=self.mock_scoped_session)
        patcher_metadata = patch('repositories.database_manager.Base.metadata', self.mock_base_metadata)
        patcher_inspect = patch('repositories.database_manager.inspect', self.mock_inspect)
